...
function equalizeBoard(board,BS){
  ...
  for(let c=0;c<NUM_COLORS;c++){
    while(counts[c]>perColor+1){
      ...
      let minC=0;for(let i=1;i<NUM_COLORS;i++) if(counts[i]<counts[minC]) minC=i;
...
function cascadeCapture(board,ownership,playerIdx,r0,c0,BS){
  ...
  function sweep(){
    ...
    for(let r=0;r<BS;r++) for(let c=0;c<BS;c++){
      ...
      for(const[r2,c2]of floodFillGroup(board,o,r,c,BS)){o[r2][c2]=playerIdx;capturedKeys.add(r2*BS+c2);seeded.add(r2*BS+c2);found=true;}
    }
    return found;
  }
  while(sweep()){}
  ...
  if(cascadeMode==="full"){
    function sweep(){
      ...
      for(let r=0;r<BS;r++) for(let c=0;c<BS;c++){
        ...
        const k=r*BS+c;if(seeded.has(k)) continue;
        if(!playerAdjacent(o,r,c,playerIdx,BS)) continue;
        for(const[r2,c2]of floodFillGroup(board,o,r,c,BS)){o[r2][c2]=playerIdx;keys.add(r2*BS+c2);seeded.add(r2*BS+c2);found=true;}
      }
      return found;
    }
    while(sweep()){}
...
function isAdjacentToPlayer(ownership,r,c,p,BS){
  return[[r-1,c],[r+1,c],[r,c-1],[r,c+1]].some(([nr,nc])=>nr>=0&&nr<BS&&nc>=0&&nc<BS&&ownership[nr][nc]===p);
}

/*
  Auto-claim adjacent same-colour cells at turn start.
  Any unclaimed neutral cell whose board colour matches the current player's chosen colour
  AND is directly adjacent to their territory gets auto-captured immediately.
  Repeats until stable (chain adjacency).
*/
function autoClaimAdjacentColour(board,ownership,playerIdx,playerDefs,BS){
  const myColorIdx=BOARD_PALETTE.indexOf(playerDefs[playerIdx].hex);
  if(myColorIdx<0) return{newOwnership:ownership,autoClaimed:new Set()};
  ...
  while(changed){
    changed=false;
    for(let r=0;r<BS;r++) for(let c=0;c<BS;c++){
      if(o[r][c]!==-1||board[r][c]!==myColorIdx) continue;
      if(!playerAdjacent(o,r,c,playerIdx,BS)) continue;
      // Capture entire connected group of this colour
      for(const[r2,c2]of floodFillGroup(board,o,r,c,BS)){
        o[r2][c2]=playerIdx;autoClaimed.add(r2*BS+c2);changed=true;
      }
    }
  }
  return{newOwnership:o,autoClaimed};
}

/*
  Enclosure detection: unclaimed neutral regions fully bordered by one player (walls count).
  Skipped if the region contains any cell matching another player's own colour.
*/
function applyEnclosures(board,ownership,playerDefs,numPlayers,BS){
  ...
  while(changed){
    ...
    for(let r0=0;r0<BS;r0++) for(let c0=0;c0<BS;c0++){
      ...
      const region=[],borderPlayers=new Set();
      const q=[[r0,c0]];visited[r0][c0]=true;
      while(q.length){
        const[r,c]=q.shift();region.push([r,c]);
        for(const[nr,nc]of[[r-1,c],[r+1,c],[r,c-1],[r,c+1]]){
          if(nr<0||nr>=BS||nc<0||nc>=BS) continue;
          if(o[nr][nc]===-1){if(!visited[nr][nc]){visited[nr][nc]=true;q.push([nr,nc]);}}
          else borderPlayers.add(o[nr][nc]);
        }
      }
      ...
      const safeToCapture=region.every(([r,c])=>{
        const ci=board[r][c];
        return playerColorIdx.every((pci,pi)=>pi===encloser||pci!==ci);
      });
      if(!safeToCapture) continue;
      for(const[r,c]of region){o[r][c]=encloser;enclosedKeys.add(r*BS+c);changed=true;}
    }
  }
...
export default function PixelWars(){
  ...
  const[turnNum,setTurnNum]            =useState(1); // absolute turn counter
  ...
  const[victoryAssured,setVictoryAssured]=useState(null); // { playerIdx, name, hex } when majority reached
  const[showScores,setShowScores]      =useState(false);  // toggle to peek at scores
  const[confirmEnd,setConfirmEnd]      =useState(false); // end-game confirm state
  // finalOwnership: snapshot saved at game end for board review
  ...
  const pickPosition=useCallback((r,c)=>{
    ...
    if(newDefs.length===numPlayers){
      ...
      // Auto-claim adjacent same-colour for player 0 at start
      const{newOwnership:o2}=autoClaimAdjacentColour(sharedBoard,o,0,newDefs,BS);
      setBoard(sharedBoard);setOwnership(o2);setPrevOwnership(null);
      setCp(0);setRound(1);setTurnNum(1);setCaptureCount(0);
      setHovered(null);setFlash(null);setEnclosedFlash(null);setAutoFlash(null);setLegendFocus(null);
      setConfirmEnd(false);setFinalOwnership(null);setReviewing(false);setVictoryAssured(null);setShowScores(false);
      setPhase("playing");
  ...
  const handleBoardClick=useCallback((r,c)=>{
    ...
    const{newOwnership:afterEnclosure,enclosed}=applyEnclosures(board,afterCapture,playerDefs,numPlayers,BS);
    ...
    // Check victory assured: any player holds strict majority of ALL cells
    if(!victoryAssured){
      const tempScores=Array(4).fill(0);
      afterEnclosure.forEach(row=>row.forEach(v=>{if(v>=0) tempScores[v]++;}));
      const TOTAL_CELLS=BS*BS;
      for(let i=0;i<numPlayers;i++){
        if(tempScores[i]>TOTAL_CELLS/2){
          setVictoryAssured({playerIdx:i,name:playerDefs[i].name,hex:playerDefs[i].hex,score:tempScores[i]});
          break;
        }
      }
    }
    if(afterEnclosure.flat().every(v=>v>=0)){
      setFinalOwnership(afterEnclosure);
      setPhase("gameover");
    }
  ...
  const endTurn=useCallback(()=>{
    ...
    let newRound=round;
    if(next===0){newRound++;if(newRound>MAX_ROUNDS){setFinalOwnership(ownership);setPhase("gameover");return;}setRound(newRound);}
    // Auto-claim adjacent same-colour for the next player before their turn begins
    const{newOwnership:o2,autoClaimed}=autoClaimAdjacentColour(board,ownership,next,playerDefs,BS);
    if(autoClaimed.size>0){
      setOwnership(o2);
      setAutoFlash(autoClaimed);
      setTimeout(()=>setAutoFlash(null),800);
      // Check if board full after auto-claim
      if(o2.flat().every(v=>v>=0)){setFinalOwnership(o2);setPhase("gameover");return;}
    }
    setCp(next);setTurnNum(n=>n+1);setCaptureCount(0);setPrevOwnership(null);setHovered(null);setLegendFocus(null);
  },[phase,cp,numPlayers,round,board,ownership,playerDefs,BS]);

  /* ── End game early ── */
  const triggerEndGame=useCallback(()=>{
    if(!confirmEnd){setConfirmEnd(true);return;}
    setFinalOwnership(ownership);setPhase("gameover");
  ...
  const resetToMenu=()=>{
    ...
    setCaptureCount(0);setLegendFocus(null);setFlash(null);setEnclosedFlash(null);setAutoFlash(null);
    setConfirmEnd(false);setFinalOwnership(null);setReviewing(false);setTurnNum(1);setVictoryAssured(null);setShowScores(false);
  };
  ...
  if(phase==="numSelect") return(
    <Shell>
      ...
      <div style={{color:"#1e1e3a",fontSize:9,letterSpacing:1,textAlign:"center",lineHeight:2.4}}>
        ...
        YOUR COLOUR AUTO-JOINS ADJACENT TO YOUR TERRITORY<br/>
    ...
    <Shell>
      ...
      {Object.entries(GRID_SIZES).map(([key,gs])=>(
        <button key={key} onClick={()=>goToColorPick(key)}
          style={{width:"100%",padding:"16px 20px",borderRadius:10,cursor:"pointer",
          ...
          <div style={{fontSize:13,fontWeight:"bold",letterSpacing:3,color:DIFFICULTIES[difficulty].color,marginBottom:4}}>
  ...
  if(phase==="playerSetup"){
    ...
    return(
      <Shell wide={pickingPos}>
        ...
        <div style={{fontSize:9,letterSpacing:3,color:"#555",marginBottom:4}}>PLAYER {setupIdx+1} OF {numPlayers}</div>
  ...
  /* ════ GAME OVER ════ */
  if(phase==="gameover"){
    const usedOwnership=finalOwnership||ownership;
    const finalScores=Array(4).fill(0);
    if(usedOwnership) for(let r=0;r<BS;r++) for(let c=0;c<BS;c++){const v=usedOwnership[r][c];if(v>=0) finalScores[v]++;}
    const winner=playerDefs[topPlayer],winHex=winner.hex;
    const solo1p=numPlayers===1,pct=Math.round(finalScores[0]/TOTAL*100);
    const grade=solo1p
      ? finalScores[0]===TOTAL?{title:"CONQUEST COMPLETE",sub:"Every cell is yours.",icon:"🏆",victory:true}
      : finalScores[0]>TOTAL*0.75?{title:"OVERWHELMING VICTORY",sub:`${pct}% captured.`,icon:"🌟",victory:true}
      : finalScores[0]>TOTAL*0.5?{title:"VICTORY",sub:`${pct}% captured.`,icon:"⚡",victory:true}
      : finalScores[0]>TOTAL*0.35?{title:"CLOSE BATTLE",sub:`${pct}% captured.`,icon:"😤",victory:false}
      :{title:"DEFEAT",sub:`Only ${pct}% captured.`,icon:"💀",victory:false}
      :null;

    if(reviewing&&usedOwnership&&board){
      return(
        <div style={{minHeight:"100vh",background:"#080812",display:"flex",flexDirection:"column",
          ...
          <div style={{fontSize:9,letterSpacing:3,color:"#444",marginBottom:4}}>FINAL BOARD</div>
          {/* Board review — static, no interaction */}
          <div style={{
            ...
            {Array.from({length:numPlayers},(_,i)=>{
              ...
              return(
                <div key={i} style={{display:"flex",alignItems:"center",gap:5,
                  ...
                  <span style={{fontSize:9,color:def.hex,letterSpacing:1}}>{def.name}</span>
                  <span style={{fontSize:10,fontWeight:"bold",color:def.hex}}>{finalScores[i]}</span>
                </div>
              );
            })}
          </div>
          <button onClick={()=>setReviewing(false)} style={{
            ...
            border:`2px solid #2a2a4a`,background:"transparent",color:"#aaa",
    ...
    return(
      <div style={{
        ...
        <style>{GLOBAL_CSS}</style>
        <div style={{fontSize:64,lineHeight:1}}>{solo1p?grade.icon:"🏆"}</div>
        <div style={{
          ...
          textAlign:"center",
          animation:!solo1p||grade.victory?"glow-pulse 2s ease-in-out infinite":"none"
        }}>
          {solo1p?grade.title:"VICTORY"}
        </div>
        {!solo1p&&(
          <div style={{fontSize:26,fontWeight:"bold",color:winHex,textAlign:"center",
            textShadow:`0 0 20px ${hexToRgba(winHex,0.6)}`}}>{winner.name}</div>
        )}
        <div style={{color:solo1p&&!grade.victory?"#FF474788":hexToRgba(winHex,0.5),
          fontSize:10,letterSpacing:1,textAlign:"center"}}>
          {solo1p?grade.sub:`P${topPlayer+1} dominates the board · ${turnNum-1} turns played`}
        </div>

        {/* Scores — always show actual numbers on game over */}
        <div style={{width:"100%",maxWidth:280}}>
          {Array.from({length:numPlayers},(_,i)=>{
            const def=playerDefs[i],isWinner=i===topPlayer&&numPlayers>1;
            const rank=numPlayers>1?[...Array(numPlayers).keys()].sort((a,b)=>finalScores[b]-finalScores[a]).indexOf(i)+1:null;
            return(
              <div key={i} style={{
                ...
                <span style={{flex:1,color:isWinner?def.hex:"#666",fontSize:10}}>{def.name}</span>
                {/* Always show exact pixel count on results screen */}
                <span style={{fontWeight:"bold",fontSize:14,color:isWinner?def.hex:"#777"}}>
                  {finalScores[i]}
                </span>
                <div style={{width:48,height:3,background:"#111",borderRadius:2,flexShrink:0}}>
                  <div style={{height:"100%",borderRadius:2,background:def.hex,
                    width:`${(finalScores[i]/TOTAL)*100}%`,boxShadow:`0 0 4px ${def.hex}`}}/>
                </div>
              </div>
            );
          })}
  ...
  return(
    <div style={{
      ...
      background:"#080812",display:"flex",flexDirection:"column",
      alignItems:"center",justifyContent:"center",
      padding:"8px 10px",fontFamily:"'Space Mono',monospace",
      color:"white",boxSizing:"border-box",gap:5,
    }}>
      <style>{GLOBAL_CSS}</style>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,width:"100%",maxWidth:380,flexShrink:0}}>
        ...
        {difficulty==="easy"&&(
          <span style={{color:"#1e1e3a",fontSize:9,letterSpacing:1,whiteSpace:"nowrap"}}>{Math.round(claimed/TOTAL*100)}%</span>
        )}
      </div>
      ...
      >
        {Array.from({length:BS*BS},(_,k)=>{
          ...
          return(
            <div key={k} onClick={()=>handleBoardClick(r,c)} onMouseEnter={()=>setHovered([r,c])}
              style={{background:bgColor,position:"relative",cursor:isClickable?"pointer":"default",
                filter:cellFilter,willChange:"filter"}}>
              {isPrev&&(
                <div style={{position:"absolute",inset:0,background:hexToRgba(curHex,0.68),
              ...
              {isLegendHighlight&&!isPrev&&!isClickable&&(
                <div style={{position:"absolute",inset:0,border:`2px solid ${BOARD_PALETTE[legendFocus]}`,
                  boxSizing:"border-box",pointerEvents:"none",opacity:0.9}}/>
              )}
              {isFlash&&(
                <div className="flash-burst" style={{position:"absolute",inset:0,background:"white",pointerEvents:"none"}}/>
              )}
              {isEnclosedFlash&&!isFlash&&(
                <div className="enclosed-burst" style={{position:"absolute",inset:0,background:"#FFD32A",pointerEvents:"none"}}/>
              )}
              {isAutoFlash&&!isFlash&&!isEnclosedFlash&&(
                <div className="auto-burst" style={{position:"absolute",inset:0,
                  background:playerDefs[cp].hex,pointerEvents:"none"}}/>
              )}
            </div>
          );
        })}
            ...
            return(
              <button key={idx} onClick={()=>setLegendFocus(isFocused?null:idx)} title={COLOR_NAMES[idx]}
                style={{flexShrink:0,width:24,height:24,borderRadius:6,cursor:"pointer",
                  background:col,border:isFocused?`2px solid white`:`2px solid transparent`,
                  boxShadow:isFocused?`0 0 10px ${col}`:"none",
                  transition:"all 0.15s",transform:isFocused?"scale(1.25)":"scale(1)",padding:0}}/>
            );
          })}
          {legendFocus!==null&&(
      ...
      {victoryAssured&&(
        <div style={{
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:victoryAssured.hex,fontWeight:"bold",fontSize:11,letterSpacing:2,marginBottom:3}}>
              {victoryAssured.name.toUpperCase()} — VICTORY ASSURED
            </div>
            <div style={{color:hexToRgba(victoryAssured.hex,0.6),fontSize:8,letterSpacing:1}}>
              Play on or call the game now?
            </div>
          </div>
          <button onClick={()=>{setFinalOwnership(ownership);setPhase("gameover");}} style={{
            ...
            color:victoryAssured.hex,fontFamily:"'Space Mono',monospace",fontSize:8,letterSpacing:1,
          }}>END NOW</button>
          <button onClick={()=>setVictoryAssured(null)} style={{
            flexShrink:0,padding:"6px 10px",borderRadius:6,cursor:"pointer",
            border:"1px solid #2a2a4a",background:"transparent",
            color:"#555",fontFamily:"'Space Mono',monospace",fontSize:8,letterSpacing:1,
          }}>PLAY ON</button>
        </div>
      )}
          ...
          return(
            <div key={i} style={{flex:1,background:isCur?hexToRgba(def.hex,0.15):"#0d0d1a",
              ...
              {showScore||showScores
                ?<div style={{fontSize:16,fontWeight:"bold",color:isCur?def.hex:"#888",lineHeight:1,
                  animation:showScores&&!showScore?"glow-pulse 1.5s ease-in-out 3":"none"}}>{scores[i]}</div>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
        ...
        {/* End game button — two-tap confirm */}
        <button onClick={triggerEndGame} style={{
          padding:"10px 14px",borderRadius:8,cursor:"pointer",
          border:`2px solid ${confirmEnd?"#FF4757":"#1a1a2e"}`,
          background:confirmEnd?"rgba(255,71,87,0.15)":"transparent",
          color:confirmEnd?"#FF4757":"#2a2a4a",
        ...
        {/* Peek at scores — only visible/useful in Normal/Hard */}
        {difficulty!=="easy"&&(
          <button onClick={()=>setShowScores(s=>!s)} style={{
            padding:"10px 12px",borderRadius:8,cursor:"pointer",
            border:`2px solid ${showScores?"#A29BFE":"#1a1a2e"}`,
            background:showScores?"rgba(162,155,254,0.15)":"transparent",
            color:showScores?"#A29BFE":"#2a2a4a",
            fontFamily:"'Space Mono',monospace",fontSize:11,
            transition:"all 0.2s",
            boxShadow:showScores?"0 0 14px rgba(162,155,254,0.4)":"none",
          }} title="Peek at scores">👁</button>
        )}
        <GhostButton onClick={resetToMenu} style={{padding:"10px 12px"}}>☰</GhostButton>
      </div>

      {confirmEnd&&(
        <div style={{color:"#FF4757",fontSize:8,letterSpacing:1,flexShrink:0}}>
          TAP CONFIRM TO END THE GAME EARLY
        </div>
      )}

      {clickableCells.size===0&&!confirmEnd&&(
        <div style={{color:"#333",fontSize:9,letterSpacing:1,flexShrink:0}}>NO MOVES — END TURN</div>
      )}
    </div>
  );
}
      ...
      {Array.from({length:BS*BS},(_,k)=>{
        ...
        return(
          <div key={k} onClick={()=>{if(!taken) onPick(r,c);}} onMouseEnter={()=>setHov(k)}
            ...
            {!taken&&isHov&&(
              <div style={{position:"absolute",inset:0,background:hexToRgba(curColor.hex,0.7),
